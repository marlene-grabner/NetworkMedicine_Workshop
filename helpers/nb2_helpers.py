...
def localized_layout(G, core, periphery, core_radius=0.35, periph_radius=0.7, seed=0):
    ...
    core_pos = (
        nx.spring_layout(core_sub, seed=seed, k=0.6)
        if core_sub.number_of_edges()
        else {
            n: (np.cos(a), np.sin(a))
            for n, a in zip(core, np.linspace(0, 2 * np.pi, len(core), endpoint=False))
        }
    )
    ...
    core_pos = {
        n: ((x - cx) / max_r * core_radius, (y - cy) / max_r * core_radius)
        for n, (x, y) in core_pos.items()
    }
    ...
    for n in periphery:
        ...
        if anchors:
            ax_, ay_ = (
                np.mean([a[0] for a in anchors]),
                np.mean([a[1] for a in anchors]),
            )
        ...
        angle += rng.normal(
            0, 0.12
        )  # jitter so nodes sharing an anchor don't stack exactly
        r = periph_radius * (1 + rng.uniform(-0.3, 0.3))
...
def plot_module_overview(
    graphs, module_seed_nodes, module_lcc_nodes, layers, proc_dir, n_context=30
):
    ...
    for ax, layer in zip(axes, layers):
        ...
        node_colors = [
            colors["dark"] if n in lcc else colors["light"] for n in sub.nodes()
        ]
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(proc_dir, "module_overview.png"), dpi=150, bbox_inches="tight"
    )
    plt.show()
